...
public class Bk extends AdminCommand
{
	...
	public void executeCommand(Player admin, String[] params)
	{
		...
		else if(params[0].equals("del"))
		{
			try
			{
				...
				PreparedStatement statement = DB.prepareStatement("DELETE FROM bookmark WHERE name = ?");
				try
				{
				...
				{
					PacketSendUtility.sendMessage(admin, "The name doesn't exists");
					return;
				}
	...
	public boolean isBookmarkExists(final String bk_name)
	{
		...
		try
		{
			s.setString(1, bk_name);
			s.setInt(2, admin.getObjectId());
			ResultSet rs = s.executeQuery();
			rs.next();
			return rs.getInt("bkcount") > 0;
		}
		catch(SQLException e)
		{
